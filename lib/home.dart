...
class _HomePageState extends State<HomePage> {
  ...
  bool detectChatbotConversation = false;
  ...
  void initStateInternal(SharedPreferencesSettingsRepository repo) {
    ...
    detectChatbotConversation = _settingsRepository.getBool("detectChatbotConversation");
  }
  ...
  void fromIntent(List<SharedMediaFile> value) async {
    ...
    if (detectChatbotConversation) {
      var chatConversation = await _url2MdConverter?.detectChatbotConversation(url: urlValue);
      if (chatConversation != null) {
        // É uma conversa de chatbot
        var frontMatter = includeFrontMatter ? chatConversation.frontMatter : "";
        var link = includeSourceLink ? chatConversation.sourceLinkSection : "";
        setState(() {
          markdown = "$frontMatter$link${chatConversation.formatConversation()}";
          markdownPreview = "$link${chatConversation.formatConversation()}";
        });
        ...
        PopupMenuButton<int>(
          ...
          itemBuilder: (context) => [
            ...
            PopupMenuItem<int>(
              value: 6,
              child: CheckedPopupMenuItem(
                checked: detectChatbotConversation,
                value: 6,
                child: const Text("Detect Chatbot Conversations"),
              ),
            ),
          ],
        ),
    ...
    setState(() {
      switch (result) {
        ...
        case 5:
          ...
          break;
        case 6:
          detectChatbotConversation = _togglePreference("detectChatbotConversation");
          break;
      }
    });
    _updateMarkdown();
  ...
  void _convert() async {
    if (detectChatbotConversation) {
      var chatConversation = await _url2MdConverter?.detectChatbotConversation(url: url);
      if (chatConversation != null) {
        // É uma conversa de chatbot
        var frontMatter = includeFrontMatter ? chatConversation.frontMatter : "";
        var link = includeSourceLink ? chatConversation.sourceLinkSection : "";
    ...
    // Se não for uma conversa de chatbot ou a detecção estiver desativada, use o fluxo normal
